...
def click(text):
    ...
    if text == 'ENTER':
        ...
        class Player:
            ...
            def __repr__(self):
                return f'\n {self.number}.{self.name} - attack {self.attack}, middle {self.middle}, defence {self.defence}, keeper {self.keeper}, left {self.left}, right {self.right}, center {self.center}, mood {self.mood}'
        ...
        if activeStr == 'GOAL':
            label.configure(text=match_stat, height=575,
                            width=1200, image=photo)
...
def soccer_mood(a_numbers_team):
    ...
    for s_pl in selected_team:
        ...
        if a_match_formation[i] == 'GK':
            a_player = ['GK', s_pl.keeper, s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] in ['SW', 'CD_1', 'CD_2']:
            a_player = [a_match_formation[i],
                        int(s_pl.defence * s_pl.center), s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] == 'LD':
            a_player = ['LD', int(s_pl.defence * s_pl.left),
                        s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] == 'RD':
            a_player = ['RD', int(s_pl.defence * s_pl.right),
                        s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] in ['DM_1', 'DM_2', 'CM_1', 'CM_2', 'AM']:
            a_player = [a_match_formation[i],
                        int(s_pl.middle * s_pl.center), s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] in ['LM', 'LW']:
            a_player = [a_match_formation[i],
                        int(s_pl.middle * s_pl.left), s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] in ['RM', 'RW']:
            a_player = [a_match_formation[i],
                        int(s_pl.middle * s_pl.right), s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] == 'LF':
            a_player = ['LF', int(s_pl.attack * s_pl.left),
                        s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] == 'CF':
            a_player = ['CF', int(s_pl.attack * s_pl.center),
                        s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
        if a_match_formation[i] == 'RF':
            a_player = ['RF', int(s_pl.attack * s_pl.right),
                        s_pl.mood, s_pl.name]
            a_match_team.append(a_player)
...
def soccer_gra(pl_match_team, cm_match_team):
    ...
    for minute in range(1, match_minute+2):
        if minute >= match_minute+1:
            print(match_stat)
        ...
        if result_2 == ['distant_strike']:
            ...
            if score_flag == 'PLAYER':
                for pl in team_attack:
                    if player_att == pl[1]:
                        distant_strike_player = [pl[0], pl[3], pl[1], pl[2]]
            ...
            if result_2 == ['player_att']:
                if score_flag == 'PLAYER':
                    match_stat.append(
                        str(minute) + '. ---GOAL! Long distant strike by ' + str(distant_strike_player))
                    result_player += 1
                    continue
                if score_flag == 'COMPUTER':
                    match_stat.append(
                        str(minute) + '. ---GOAL! Long distant strike by your opponent')
                    result_computer += 1
                    continue
                ...
                if score_flag == 'PLAYER':
                    match_stat.append(
                        str(minute) + '. Long distant strike was saved by your keeper')
                if score_flag == 'COMPUTER':
                    match_stat.append(
                        str(minute) + ". Long distant strike was saved by opponent's keeper")
                continue
        ...
        if not result_3:  # если полузащита не была переброшена дальним пасом
            ...
            if result_3 == ['middle_strike']:
                ...
                if score_flag == 'PLAYER':
                    for pl in team_attack:
                        if player_att == pl[1]:
                            middle_strike_player = [pl[0], pl[3], pl[1], pl[2]]
                # выбирается мешающий ему игрок
                ...
                if result_3 == ['player_att']:
                    if score_flag == 'PLAYER':
                        match_stat.append(
                            str(minute) + '. ---GOAL! Middle distant strike by' + str(middle_strike_player))
                        result_player += 1
                        continue
                    if score_flag == 'COMPUTER':
                        match_stat.append(
                            str(minute) + '. ---GOAL! Middle distant strike by your opponent')
                        result_computer += 1
                        continue
                    ...
                    if score_flag == 'PLAYER':
                        match_stat.append(
                            str(minute) + '. Middle distant strike was saved by your keeper')
                    if score_flag == 'COMPUTER':
                        match_stat.append(
                            str(minute) + ". Middle distant strike was saved by opponent's keeper")
                    continue
        ...
        if result_4 == ['defence_crush']:  # провал обороны с выходом группы атаки на ворота
            ...
            if result_4 == 'att_4_sum':
                if score_flag == 'PLAYER':  # случайный выбор автора голевого удара
                    ...
                    for pl in team_attack:
                        if player_goal == pl[1]:
                            close_attack_player = [pl[0], pl[3], pl[1], pl[2]]
                    match_stat.append(
                        str(minute) + '.---GOAL! After defence crush by' + str(close_attack_player))
                    result_player += 1
                if score_flag == 'COMPUTER':
                    match_stat.append(
                        str(minute) + '.---GOAL! After defence crush by your opponent')
                    result_computer += 1
            else:
                if score_flag == 'PLAYER':
                    match_stat.append(
                        str(minute) + '. Brilliant save after defence crush by your keeper!')
                if score_flag == 'COMPUTER':
                    match_stat.append(
                        str(minute) + ". Save after defence crush by opponent's keeper")
        if result_4 == ['face_to_face']:  # выбирается игрок, вышедший на вратаря
            ...
            if result_4 == ['player_att']:
                if score_flag == 'PLAYER':
                    match_stat.append(
                        str(minute) + '. ---GOAL! After face-to-face by' + str(close_attack_player))
                    result_player += 1
                if score_flag == 'COMPUTER':
                    match_stat.append(
                        str(minute) + '. ---GOAL! After face-to-face by your opponent')
                    result_computer += 1
            else:
                if score_flag == 'PLAYER':
                    match_stat.append(
                        str(minute) + '. Brilliant save after face-to-face by your keeper!')
                if score_flag == 'COMPUTER':
                    match_stat.append(
                        str(minute) + ". Save after face-to-face by opponent's keeper")
        if result_4 == ['close_strike']:  # выбирается бьющий игрок
            ...
            if result_4 == ['player_att']:
                if score_flag == 'PLAYER':
                    match_stat.append(
                        str(minute) + '. ---GOAL! Close strike by' + str(close_attack_player))
                    result_player += 1
                if score_flag == 'COMPUTER':
                    match_stat.append(
                        str(minute) + '. ---GOAL! Close strike by your opponent')
                    result_computer += 1
                ...
                if score_flag == 'PLAYER':
                    match_stat.append(
                        str(minute) + '. Close strike was saved by your keeper!')
                if score_flag == 'COMPUTER':
                    match_stat.append(
                        str(minute) + ". Close strike was saved by opponent's keeper")
...
photo = PhotoImage(file='football_PNG52764.png')
root['bg'] = 'grey'
